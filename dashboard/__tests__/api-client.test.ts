...
import { fetchIncident, fetchIncidents, fetchPolicies, fetchTransactions } from "@/lib/api/client";
...
describe("api client mock data", () => {
  ...
  it("filters and paginates transactions by policy key", async () => {
    ...
    expect(firstPage.items.every((item) => item.policyPubkey === policyPubkey)).toBe(true);
